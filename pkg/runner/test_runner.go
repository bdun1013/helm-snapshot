...
func (tr *TestRunner) getTestSuites(chartPath, chartRoute string, chart *chart.Chart) ([]*suite.TestSuite, error) {
	...
	resultSuites := make([]*suite.TestSuite, 0, len(filesSet))
	for file := range filesSet {
		s, err := suite.ParseTestSuiteFile(file, chartRoute)
		if err != nil {
			tr.handleSuiteResult(&suite.TestSuiteResult{
				FilePath:  file,
				ExecError: err,
			})
			continue
		}
		resultSuites = append(resultSuites, s)
	}
...
func (tr *TestRunner) runSuitesOfChart(suites []*suite.TestSuite, chart *chart.Chart) bool {
	chartPassed := true
	for _, s := range suites {
		snapshotCache, err := snapshot.CreateSnapshotOfSuite(s.DefinitionFile, tr.Config.UpdateSnapshot)
		if err != nil {
			tr.handleSuiteResult(&suite.TestSuiteResult{
				FilePath:  s.DefinitionFile,
				ExecError: err,
			})
			continue
		}

		result := s.Run(chart, snapshotCache, &suite.TestSuiteResult{})
...
func (tr *TestRunner) handleSuiteResult(result *suite.TestSuiteResult) {
...
func (tr *TestRunner) countSuite(suite *suite.TestSuiteResult) {
	if suite.Passed {
...
func (tr *TestRunner) countTest(test *suite.TestJobResult) {
	if test.Passed {
