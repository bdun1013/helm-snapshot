...
) (bool, []*assertion.AssertionResult) {
	testPass := true
	assertsResult := make([]*assertion.AssertionResult, len(t.Assertions))

	for idx, a := range t.Assertions {
		result := a.Assert(
			manifestsOfFiles,
			snapshotComparer,
			&assertion.AssertionResult{Index: idx},
		)
